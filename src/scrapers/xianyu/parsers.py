"""闲鱼站点的 JSON 解析器(mtop 接口)。

从原 src/parsers.py 完整搬入,保留全部现有语义。
产出的商品/卖家字典的键(如 `商品标题`、`卖家昵称` 等)是全局约定,
下游 result_storage / notification / AI 层均依赖这套键名,不得改动。
"""
...
from src.scrapers.xianyu.constants import FLEAMARKET_SCHEME
from src.utils import safe_get


async def parse_search_results_json(json_data: dict, source: str) -> list:
    ...
    try:
        ...
        for item in items:
            ...
            page_data.append({
                ...
                "发货地区": area,
                "卖家昵称": seller,
                "商品链接": raw_link.replace(FLEAMARKET_SCHEME, "https://www.goofish.com/"),
...
async def parse_user_items_data(items_json: list) -> list:
...
# 别名以向后兼容之前 `from src.parsers import _parse_search_results_json` 的 import
_parse_search_results_json = parse_search_results_json
_parse_user_items_data = parse_user_items_data
